...
vec3 calcMove(in vec3 pos, in float f0, in float f1, in float f2, in float f3, in float f4, in float f5, in vec3 amp1, in vec3 amp2) {
    ...
    float day = 1.6 * TimeNoon;
	float sunrise = 1.0 * TimeSunrise;
	float sunset = 1.0 * TimeSunset;
	float night = 1.8 * TimeMidnight; 
	float rain = rainStrength*2.5;
...
void main() {
	...
	*/
	...
	if ( mc_Entity.x == ENTITY_TALLGRASS)
			position.xyz += calcMove(worldpos.xyz, 0.0041, 0.0070, 0.0044, 0.0038, 0.0063, 0.0000, vec3(0.8,0.0,0.8), vec3(0.8,0.0,0.48));
	#endif
	...
	if ( mc_Entity.x == ENTITY_WHEAT)
			position.xyz += calcMove(worldpos.xyz, 0.0041, 0.0070, 0.0044, 0.0038, 0.0240, 0.0000, vec3(0.8,0.0,0.8), vec3(0.8,0.0,0.8));
	#endif
	#ifdef WAVING_FIRE
