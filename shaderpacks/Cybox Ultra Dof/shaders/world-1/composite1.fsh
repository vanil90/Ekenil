...
//----------Sky----------//
	//#define GODRAYS			//in this step previous godrays result is blurred
		const float exposure = 10.0;	      //godrays intensity
		const float density = 1.0;
		const int NUM_SAMPLES = 13;			//increase this for better quality at the cost of performance
		const float grnoise = 0.0;		   //amount of noise

	 #define Clouds
		#define CLOUD_SIZE 1.0
...
vec3 calcFog(vec3 fposition, vec3 color, vec3 fogclr) {
  ...
  float fog = min(exp(-length(fposition)/density/(sunVisibility*0.7+0.3))+start*sunVisibility*(1-rainStrength),1.0);
...
//Colour Properties
	...
	reflection.a = min(reflection.a + 1.0*sky_lightmap,0.5);
