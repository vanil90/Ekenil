...
//----------Sky----------//
	#define GODRAYS			//in this step previous godrays result is blurred
		const float exposure = 12.0;	      //godrays intensity
...
#define Specular_REFLECTION_STRENGTH 3.0
...
for (int i = 0;i<10;i++) {
  ...
  cl = max(cl-(abs(i-5.)/5.)*0.01,0.)*0.08;
	/*--------------------------------*/
	totalcloud += vec4(c.rgb*exp(-totalcloud.a),cl);
	totalcloud.a = min(totalcloud.a,1.0);
	/*--------------------------------*/
...
//vec3 bcolor = color/MAX_COLOR_RANGE;
	vec2 velocity = (currentPosition - previousPosition).st * 0.04;
...
if (iswater > 0.9 && isEyeInWater == 0) {
	...
	float fresnel_SPEC = clamp(pow(1.0 + normalDotEye, 1.0),0.0,1.0);
								 ...
								 if (land < 0.9 && hand < 0.1) {
										...
										vec3 sky_color = calcFog(reflectedVector,drawCloud(reflectedVector,vec3(0.0)),skyc)*clamp(sky_lightmap*2.0-2/0.0,0.0,0.0);

										if(specmap*fresnel_SPEC > 0.002)
										reflection = raytraceGround(fragpos, normal, skyc);

										reflection.rgb = mix(sky_color, reflection.rgb, reflection.a)+(color.a)*lc*(1000.0-rainStrength)*24.;
										reflection.rgb = mix(reflection.rgb,reflection.rgb*normalize(color.rgb),0.5);
										reflection.a = min(reflection.a,1.0);
										reflection.rgb = reflection.rgb/Specular_REFLECTION_STRENGTH;
