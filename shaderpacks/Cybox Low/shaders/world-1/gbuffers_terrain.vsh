...
vec3 calcMove(in vec3 pos, in float f0, in float f1, in float f2, in float f3, in float f4, in float f5, in vec3 amp1, in vec3 amp2) {
    ...
    float day = 2.0 * TimeNoon;
	float sunrise = 2.0 * TimeSunrise;
	float sunset = 2.0 * TimeSunset;
	float night = 2.0 * TimeMidnight;
	float rain = rainStrength*1.0;
...
void main() {
	...
	*/
	...
	if ( mc_Entity.x == ENTITY_FIRE)
			position.xyz += calcMove(worldpos.xyz, 0.0105, 0.0096, 0.0087, 0.0063, 0.0097, 0.0000, vec3(1.5,0.0,1.5), vec3(0.8,0.8,0.8));
	#endif
	}
	#ifdef WAVING_LAVA
	if ( mc_Entity.x == ENTITY_LAVAFLOWING || mc_Entity.x == ENTITY_LAVASTILL ) {
			mat = 0.4;
			position.xyz += calcWaterMove(worldpos.xyz) * 0.80;
			}
	#endif
