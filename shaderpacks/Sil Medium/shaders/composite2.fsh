...
vec3 drawSun(vec3 fposition, vec3 color, float vis) {
	...
	float sun = exp(-angle*angle*angle);
			sun *= (1.0-rainStrength*0.75)*sunVisibility;
...
vec3 drawMoon(vec3 fposition, vec3 color, float vis) {
	...
	float moon = exp(-angle*angle*angle);
			moon *= (1.0-rainStrength*0.75)*moonVisibility;
...
vec4 raytrace(vec3 fragpos, vec3 normal, vec3 fogclr, vec3 rvector) {
    ...
    for(int i=0;i<25;i++){
        ...
        vec3 wfragpos1 = vec3(pos.st, texture2D(depthtex1, pos.st).r);
        wfragpos1 = nvec3(gbufferProjectionInverse * nvec4(wfragpos1 * 2.0 - 1.0));
        float err = distance(fragpos,wfragpos1);
		if(err < pow(length(rvector)*1.85,1.15)){
                sr++;
                if(sr >= maxf){
					bool land = texture2D(depthtex1, pos.st).r < comp;
                    if (isEyeInWater == 0) color.rgb = getcolor(pos.st, 1);
					color.rgb = land ? mix(color.rgb,fogclr*(0.7+0.3*tmult)*(2.0-rainStrength*1.0)*4.,calcFog(fragpos)) : drawSun(rvector,fogclr, 1.0)*(1.0-isEyeInWater);
...
if (transparency) {
	...
	//Water refraction
	if (iswater && isEyeInWater == 0.0) {
		vec3 wpos = (gbufferModelViewInverse*fragpos0).rgb+cameraPosition;

		float ft = frameTimeCounter*4.0;

		wpos.x += sin(wpos.z+ft)*0.25;
		wpos.z += cos(wpos.x+ft*0.5)*0.25;
		...
		vec2 refractcoord = texcoord.xy + refract.xy*refMult;
		c = getcolor(refractcoord, 0);

		//Caustics
		float getlight = (eyeBrightness.y / 255.0);
		float caustics = waterH((wpos.xyz)*2.0,ft)*0.2+1.1;
		if(getlight > 0.1)c *= caustics*caustics*caustics;
		/*---------------------------------------------------*/
	}
	...
	if (reflective && isEyeInWater == 0.0) {
		...
		vec4 reflection = raytrace(fragpos0.xyz, normal, sky_c, reflectedVector);
